...
# Additions with auto completion by Murat Gueven <muratg@online.de>
# V1.2 (autocompletion versioning) for Zim 0.61
#
# Additions regaridn tasks by theoky.ctxt@gmail.com
# v1.3 Task List++: Agenda, template tasks, future tasks
...
_futuredate_re = re.compile(r'\s*\[f:(.+)\]')
...
V1.2 (autocompletion entry included)
v1.3 Task List++: Agenda, template tasks, future tasks
...
'''), # T: plugin description
	...
	plugin_preferences = (
		...
		('inactive_tasks_label', 'string', _('Labels for agenda and template tasks'), 'Agenda, Template', StringAllowEmpty),
			# T: label for plugin preferences dialog - label is by default "Agenda"
	...
	_rebuild_on_preferences = ['all_checkboxes', 'labels', 'next_label', 'inactive_tasks_label', 'deadline_by_page', 'nonactionable_tags',
...
class IndexExtension(ObjectExtension):
	...
	def __init__(self, plugin, index):
		...
		self.inactive_tasks_label = None
		self.inactive_tasks_label_re = None
	...
	def _set_preferences(self):
		...
		if self.preferences['inactive_tasks_label']:
			self.inactive_tasks_label = [
				s.strip() # tolower()?
					for s in self.preferences['inactive_tasks_label'].split(',')
						if s and not s.isspace()
			]
			self.task_labels.extend(self.inactive_tasks_label)
			regex = r'^(' + '|'.join(map(re.escape, self.inactive_tasks_label)) + r')(?!\w)'
			self.inactive_tasks_label_re = re.compile(regex)
		else:
			self.inactive_tasks_label = []
			self.inactive_tasks_label_re = None
...
class TasksParser(Visitor):
	...
	def __init__(self, task_label_re, next_label_re, inactive_tasks_label_re, nonactionable_tags, all_checkboxes, defaultdate):
		self.task_label_re = task_label_re
		self.next_label_re = next_label_re
		self.inactive_tasks_label_re = inactive_tasks_label_re
		...
		self._in_agenda_list = False # True if agenda
		self._prio = 0 # > 0 if a header of a tasklist has the form TODO_: @tag1 ! 
	...
	def _check_para_start(self, strings):
		...
		and self._matches_label(line):
			...
			if all((w.startswith('@') or not(re.search(no_prio_pattern, w))) for w in words):
				self._intasklist = True
				self._tasklist_tags = set(w.strip('@') for w in words if w.startswith('@'))
				self._prio = line.count('!')
				if self._matches_inactive_tasks_label(line):
					self._in_agenda_list = True
			else:
				self._intasklist = False
		else:
			self._intasklist = False
	...
	def _parse_para_text(self, strings):
		...
		for line in u''.join(strings).splitlines():
			if self._matches_label(line) and not self._matches_inactive_tasks_label(line):
	...
	def _matches_inactive_tasks_label(self, line):
		return self.inactive_tasks_label_re and self.inactive_tasks_label_re.match(line)
	...
	def _parse_task(self, text, open=True):
		...
		future_date_text = _NO_DATE
		future_date = None
		datematch = _futuredate_re.search(text) # first match
		if datematch:
			future_date = parse_date(datematch.group(0))
			if future_date:
				future_date_text = '%04i-%02i-%02i' % future_date # (y, m, d)
		...
		actionable = True
		if due == _NO_DATE:
			if future_date_text == _NO_DATE:
				due = parent[3] # default to parent date (or default for root)
			else:
				if datetime.now() < datetime.datetime(future_date[0], future_date[1], future_date[2], tzinfo=datetime.LocalTimezone()):
					actionable = False
...
class TaskListDialog(Dialog):
	...
	def __init__(self, window, index_ext, preferences):
		Dialog.__init__(self, window, _('Task List++ (1.3)'), # T: dialog title
		...
		# Tag auto completion
		hbox.pack_start(gtk.Label(_('Tags')+': '), False) # T: Input label
		self.tag_entry = InputEntry()
		# TODO: set to 'all tags' when cleared
		self.tag_entry.set_icon_to_clear()
		hbox.pack_start(self.tag_entry)

		# Tag list added 'self.tag_entry' to pass for auto completion entry
		self.tag_list = TagListTreeView(self.index_ext, self.task_list, self.tag_entry)
...
class TagListTreeView(SingleClickTreeView):
	...
	def __init__(self, index_ext, task_list, tag_entry):
		...
		tag_entry.set_completion(self.completion)
		...
		tag_entry.connect('activate', self.activate_return)
	...
	def activate_return(self, entry):
		...
		# TODO: This is another 'hidden' functionality for an easy why to show them
		# but it needs to be implemented in a proper way
		if entry.get_text() == "":
			return
		tags = []
		tags.append(entry.get_text())
		# there can be still a additional filter set
		labels = self.get_labels()
		self.task_list.set_tag_filter(tags, labels)
